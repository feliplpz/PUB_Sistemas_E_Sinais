"""
Testes para o modelo Predador-Presa de Lotka-Volterra.
"""
...
from lotka_volterra import lotka_volterra, resolver_sistema, calcular_equilibrio


class TestLotkaVolterra:
    """Testes para a função lotka_volterra."""
...
class TestResolverSistema:
    ...
    def test_retorna_tres_arrays(self):
        ...
        result = resolver_sistema(6.0, 2.0, 2.0, 3.0, 1.0, 1.0, t_max=10.0, n_points=100)
        assert len(result) == 3
    ...
    def test_tamanho_correto_dos_arrays(self):
        ...
        n_points = 100
        t, x1, x2 = resolver_sistema(6.0, 2.0, 2.0, 3.0, 1.0, 1.0, 
                                      t_max=10.0, n_points=n_points)
    ...
    def test_oscilacao_periodica(self):
        ...
        t, x1, x2 = resolver_sistema(6.0, 2.0, 2.0, 3.0, 1.0, 1.0, t_max=20.0, n_points=2000)
        
        x1_max = np.max(x1)
        x1_min = np.min(x1)
        
        assert x1_max > x1[0], "População de presas deve oscilar acima do valor inicial"
        assert x1_min < x1[0], "População de presas deve oscilar abaixo do valor inicial"
    ...
    def test_defasagem_temporal(self):
        ...
        assert idx_max_x2 > idx_max_x1, "Pico de predadores deve ocorrer após pico de presas"
...
class TestIntegracaoSistema:
    ...
    def test_equilibrio_e_estavel_no_ponto_de_equilibrio(self):
        ...
        t, x1, x2 = resolver_sistema(alpha, beta, gamma, delta, x1_eq, x2_eq, 
                                      t_max=5.0, n_points=100)
    ...
    def test_conservacao_energia_orbita_fechada(self):
        ...
        t, x1, x2 = resolver_sistema(6.0, 2.0, 2.0, 3.0, 1.0, 1.0, t_max=50.0, n_points=5000)
        ...
        assert len(indices_proximos) > 1, "Sistema deve retornar próximo ao ponto inicial (órbita fechada)"
...
class TestCasosEspeciais:
    ...
    def test_tempo_muito_pequeno(self):
        ...
        t, x1, x2 = resolver_sistema(6.0, 2.0, 2.0, 3.0, 1.0, 1.0, t_max=0.01, n_points=2)
        assert len(t) >= 1
        assert_allclose(t[0], 0.0)
    ...
    def test_parametros_muito_grandes(self):
        ...
        t, x1, x2 = resolver_sistema(100.0, 50.0, 50.0, 100.0, 1.0, 1.0, t_max=1.0, n_points=1000)
        assert np.all(np.isfinite(x1))
        assert np.all(np.isfinite(x2))
